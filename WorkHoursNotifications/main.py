...
def time_formatter(start_time, end_time):
    start_hours, start_minuts, start_seconds = start_time.split(':')
    end_hours, end_minuts, end_seconds = end_time.split(':')

    # print(int(end_hours)-int(start_hours))
    return f'{(int(end_hours)-int(start_hours))}:{(int(end_minuts)-int(start_minuts))}'
